try:
    from data.SnappyHexOptimise import BasicDuctRun
except:
    from .data.SnappyHexOptimise import BasicDuctRun
try:
...
class KaplanDuct(Problem, ControlPolygonInterface):

    def __init__(self, settings):
        self.source_case = settings.get('source_case', 'data/KaplanDuct/case_fine')
        self.case_path = settings.get('case_path', 'data/KaplanDuct/case_single/')
        self.domain_files = settings.get('boundary_files', ['data/KaplanDuct/boundary_1stspline.csv', \
                            'data/KaplanDuct/boundary_2ndspline.csv'])
        self.fixed_points_files = settings.get('fixed_points_files', ['data/KaplanDuct/fixed_1.csv',\
                            'data/KaplanDuct/fixed_2.csv'])
        self.n_control = settings.get('n_control', [1, 1])
        self.niter = settings.get('niter', 5)
        self.thickness = settings.get('thickness', np.array([0, 0, 1000]))
    ...
    def setup(self, verbose=False):
        ...
        problem = BasicDuctRun(case_path=self.case_path)
...
if __name__=='__main__':
    ...
    for i in range(x.shape[0]):
        if prob.constraint(x[i]):
            rand_x.append(x[i])
    res = prob.evaluate(rand_x[10], verbose=True)
    print(res)
